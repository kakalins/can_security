...
ABSOLUTE_MODELS_PATH = os.path.join(ABSOLUTE_PATH, 'models/car_hacking/online_if/complete/')
ABSOLUTE_DATASET_PATH = os.path.join(ABSOLUTE_PATH, 'dataset/preprocessed/car_hacking/')
ABSOLUTE_PLOTS_PATH = os.path.join(ABSOLUTE_PATH, 'plots/car_hacking/online_if/complete/roc_auc/')
...
print("--- Iniciando a geração dos gráficos ROC AUC para os modelos Online iForest (car_hacking) ---")
...
for dataset_name in DATASET_NAMES:
    ...
    model_file_path = os.path.join(ABSOLUTE_MODELS_PATH, f'best_online_if_car_hacking_{dataset_name}.pkl')
    try:
        with open(model_file_path, 'rb') as f:
            model_info = pickle.load(f)
    except FileNotFoundError:
        print(f"  AVISO: Arquivo de modelo não encontrado para '{dataset_name}'. Pulando...")
        continue
    ...
    plt.title(f'Curva ROC para OnlineIF_{feature_set_name} - Dataset {dataset_name}')
    plt.legend(loc="lower right")
    
    plot_output_path = os.path.join(ABSOLUTE_PLOTS_PATH, f'roc_auc_online_if_{feature_set_name}_{dataset_name}.png')
