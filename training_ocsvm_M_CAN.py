...
parser = argparse.ArgumentParser(description='Treina OCSVM com tunagem de hiperparâmetros.')
...
ABSOLUTE_MODELS_PATH = ABSOLUTE_PATH + 'models/M_CAN/'
ABSOLUTE_PLOTS_PATH = ABSOLUTE_PATH + 'plots/M_CAN/'
...
if COMPLETE_TRAINING:
    ...
    log_file_path = os.path.join(ABSOLUTE_MODELS_PATH, 'complete/ocsvm_summary_M_CAN.txt')
    with open(log_file_path, 'w') as log_file:
        log_file.write("Resumo do treinamento dos modelos OCSVM\n")
        log_file.write("=" * 40 + "\n")
    ...
    # --- Preparando o arquivo de log para o resumo ---
    log_file_path = os.path.join(ABSOLUTE_MODELS_PATH, 'fast/ocsvm_summary_M_CAN.txt')
    with open(log_file_path, 'w') as log_file:
        log_file.write("Resumo do treinamento dos modelos OCSVM\n")
        log_file.write("=" * 40 + "\n")
...
# Tunagem de hiperparâmetros do OCSVM
...
y_test = classes_test.apply(lambda c: 0 if c == '0' else 1)
...
for f_idx in range(len(feature_sets)):
        ...
        for n in NU:
            for g in GAMMA:
                for fold in range(FOLD_SPLIT):
                    ...
                    y_val = classes_val.apply(lambda c: 0 if c == '0' else 1)
                    ...
                    print(f"M_CAN - Modelo: OCSVM_{feature_set_name} | Kernel: {k} | Nu: {n} | Gamma: {g} | F1-Score: {score}")
    ...
    if best_model_for_set is None:
        print(f"--> Nenhum modelo com F1-Score > 0 foi encontrado para {feature_set_name}. Pulando para o próximo.")
        with open(log_file_path, 'a') as log_file:
            log_file.write(f"--> Nenhum modelo com F1-Score > 0 foi encontrado para {feature_set_name}. Pulando para o próximo.\n")
    ...
    print("-" * 30)
    with open(log_file_path, 'a') as log_file:  
        log_file.write(f"Melhor modelo para {feature_set_name}: F1-Score: {best_f1_for_set:.4f} | Acc: {metrics_for_set['acc']:.4f}, TPR: {metrics_for_set['tpr']:.4f}, FPR: {metrics_for_set['fpr']:.4f}\n")
        log_file.write(f"    com {best_model_for_set.kernel} kernel, nu {best_model_for_set.nu}, gamma {best_model_for_set.gamma}.\n")
        log_file.write("-" * 30 + "\n")
...
print("-" * 30)
print(f"\nMelhor modelo geral: {overall_best_feature_set} com kernel: {overall_best_model.kernel}, nu: {overall_best_model.nu}, gamma: {overall_best_model.gamma} | Melhor F1-Score na Validação: {overall_best_f1:.4f}")
print("-" * 30)
with open(log_file_path, 'a') as log_file:  
    log_file.write(f"\nMelhor modelo geral: {overall_best_feature_set} com kernel: {overall_best_model.kernel}, nu: {overall_best_model.nu}, gamma: {overall_best_model.gamma} | Melhor F1-Score na Validação: {overall_best_f1:.4f}\n")
    log_file.write("-" * 30 + "\n")
...
if COMPLETE_TRAINING:
    os.makedirs(ABSOLUTE_MODELS_PATH + 'complete/', exist_ok=True)
    output_models_file = f'{ABSOLUTE_MODELS_PATH}complete/best_ocsvm_models_M_CAN.pkl'
else:
    os.makedirs(ABSOLUTE_MODELS_PATH + 'fast/', exist_ok=True)
    output_models_file = f'{ABSOLUTE_MODELS_PATH}fast/best_ocsvm_models_M_CAN.pkl'
...
for i, (feature_set_name, model_info) in enumerate(best_models.items()):
    ...
    ax.set_title(f'Matriz de Confusão (Teste) para ocsvm_{feature_set_name} - Fuzzy Attack\nF1-Score: {f1:.4f}')
...
if COMPLETE_TRAINING:
    ...
    plt.savefig(f'{ABSOLUTE_PLOTS_PATH}complete/confusion_matrices_test_ocsvm_M_CAN.png')
    print(f"\nMatrizes de confusão para cada conjunto de features salvas em 'plots/car_hacking/complete/confusion_matrices_test_ocsvm_M_CAN.png'")
else:
    os.makedirs(ABSOLUTE_PLOTS_PATH + 'fast/', exist_ok=True)
    plt.savefig(f'{ABSOLUTE_PLOTS_PATH}fast/confusion_matrices_test_ocsvm_M_CAN.png')
    print(f"\nMatrizes de confusão para cada conjunto de features salvas em 'plots/car_hacking/fast/confusion_matrices_test_ocsvm_M_CAN.png'")
