# Esta versão do código implementa a tunagem de hiperparâmetros do Extended Isolation Forest
...
COMPLETE_TRAINING = False  # Defina como True para tunagem completa, False para teste rápido com poucos hiperparâmetros
...
if COMPLETE_TRAINING:
  ...
  _max_samples = [100, 256]
  _contamination = [0.0085, 0.0088, 0.009]
  _bootstrap = [False]
  _extension_level = [1, 2, 3] # Novo hiperparâmetro para EIF
  ...
  _max_samples = [200, 256]
  _contamination = [0.0088, 0.009]
  _bootstrap = [False]
  _extension_level = [1, 2]
...
print("\nIniciando a tunagem de hiperparâmetros do Extended Isolation Forest...")
print("-" * 30)
for f_idx in range(len(feature_sets)):
                ...
                for b in _bootstrap:
                    for ext in _extension_level:
                        for fold in range(FOLD_SPLIT):
                            ...
                            # O extension_level não pode ser maior que o número de features - 1
                            current_ext_level = min(ext, X_train.shape[1] - 1)
                            if current_ext_level < 1: current_ext_level = 1

                            # Treinamento do modelo Extended Isolation Forest
                            model_eiforest = ExtendedIsolationForest(n_estimators = n, max_samples = s, contamination = c, bootstrap = b, extension_level=current_ext_level, random_state=RANDOM_SEED).fit(norm_X_train)

                            val_anomaly_preds = model_eiforest.predict(norm_X_val)
                            val_anomaly_preds[val_anomaly_preds == 1] = 0
                            val_anomaly_preds[val_anomaly_preds == -1] = 1

                            score = f1_score(y_val, val_anomaly_preds)
                            print(f"{DATASET_NAME} - Modelo: EIF_{feature_set_name} | n_est: {n}, max_s: {s}, cont: {c}, boot: {b}, ext: {current_ext_level} | F1: {score:.4f}")

                            if score > best_f1_for_set:
                                best_f1_for_set = score
                                best_model_for_set = model_eiforest
    ...
    print(f"    com {best_model_for_set.n_estimators} estimadores, contaminação {best_model_for_set.contamination}, ext_level {best_model_for_set.extension_level}")
...
print("-" * 30)
print(f"\nMelhor modelo geral para {DATASET_NAME}: {overall_best_feature_set} com estimator: {overall_best_model.n_estimators}, contamination: {overall_best_model.contamination}, ext_level: {overall_best_model.extension_level} | Melhor F1-Score na Validação: {overall_best_f1:.4f}")
print("-" * 30)

# --- Salvando os melhores modelos em um arquivo ---
output_models_file = f'best_eif_models_v6_{DATASET_NAME}.pkl'
...
for i, (feature_set_name, model_info) in enumerate(best_models.items()):
    ...
    ax.set_title(f'Matriz de Confusão (Teste) para EIF_{feature_set_name} - {DATASET_NAME} Attack\nF1-Score: {f1:.4f}')
...
plt.tight_layout()
plt.savefig(f'confusion_matrices_test_eif_v6_{DATASET_NAME}.png')
print(f"\nMatrizes de confusão para cada conjunto de features salvas em 'confusion_matrices_test_eif_v6_{DATASET_NAME}.png'")
# plt.show()
