...
COMPLETE_TRAINING = True  # Defina como True para tunagem completa, False para teste rápido com poucos hiperparâmetros
...
if COMPLETE_TRAINING:
  ...
  FOLD_SPLIT = 10
  RANDOM_SEED = 33
  DATASET_NAME = 'Gear'  # Options: 'Fuzzy', 'Gear', 'RPM'
  FEATURE_SET_SAHIFT = 6  # Options: 'F1' to 'F8'
  ...
  # _contamination = ['auto']
  _contamination = ['auto', 0.0078, 0.008, 0.0082, 0.0085, 0.0086, 0.0088, 0.009, 0.0092]
  ...
  _contamination = ['auto',0.0088, 0.009]
  _bootstrap = [False]
            ...
            for c in _contamination:
                for b in _bootstrap:
                    for fold in range(FOLD_SPLIT):
                        ...
                        print(f"{DATASET_NAME} - Modelo: iForest_{feature_set_name} | Estimator: {n} | Max Samples: {s} | Contamination: {c} | Bootstrap: {b} | F1-Score: {score}")
...
print("-" * 30)
print(f"\nMelhor modelo geral para {DATASET_NAME}: {overall_best_feature_set} com estimator: {overall_best_model.n_estimators}, contamination: {overall_best_model.contamination} | Melhor F1-Score na Validação: {overall_best_f1:.4f}")
print("-" * 30)

# --- Salvando os melhores modelos em um arquivo ---
output_models_file = f'models/best_iforest_models_v6_{DATASET_NAME}.pkl'
...
for i, (feature_set_name, model_info) in enumerate(best_models.items()):
    ...
    # print(f"Matriz de Confusão para iForest_{feature_set_name} no conjunto de teste:\n{cm}\n")
    
    # Criar rótulos com contagem e percentual
    group_counts = [f'{value:0.0f}' for value in cm.flatten()]
    # group_percentages = [f'{value:.2%}' for value in cm.flatten()/np.sum(cm)]
    group_percentages = [cm[0][0]/total_benign, cm[0][1]/total_benign, cm[1][0]/total_malicious, cm[1][1]/total_malicious]
    group_percentages = [f'{value*100:.2f}%' for value in group_percentages]
    annot_labels = [f'{v1}\n{v2}' for v1, v2 in zip(group_counts, group_percentages)]
    ...
    ax.set_title(f'Matriz de Confusão (Teste) para iForest_{feature_set_name} - {DATASET_NAME} Attack\nF1-Score: {f1:.4f}')
...
plt.tight_layout()
plt.savefig(f'plots/confusion_matrices_test_if_v6_{DATASET_NAME}.png')
print(f"\nMatrizes de confusão para cada conjunto de features salvas em 'plots/confusion_matrices_test_if_v6_{DATASET_NAME}.png'")
# plt.show()
print("-" * 30)


################################
# A matriz de confusao precisa imprimir o percentual com base no total 
# de ocorrencias da classe no conjunto de teste e nao do conjunto todo
# O real malicioso previsto tem que ser em cima do total de malicioso apenas
# e esse total é o contido no conjunto de teste.
################################
