...
for file in file_list:
    ...
    # --- Divisão sequencial dos dados ---
    
    # 1. Conjunto de Treino: os primeiros N_TRAINING_SAMPLES dados benignos ('R')
    df_train = df[df['Label'] == 'R'].head(N_TRAINING_SAMPLES)
    
    # Encontra o índice do último dado de treino no dataframe original
    last_train_index = df_train.index[-1]
    
    # 2. Conjunto de Validação: os primeiros N_VAL_SAMPLES dados maliciosos ('T') APÓS o conjunto de treino
    df_after_train = df.loc[last_train_index + 1:]
    X_val_attack = df_after_train[df_after_train['Label'] == 'T'].head(N_VAL_SAMPLES)
    
    # Encontra o índice do último dado de validação no dataframe original
    last_val_index = X_val_attack.index[-1]
    
    # 3. Conjunto de Teste: todos os dados restantes APÓS o conjunto de validação
    df_test = df.loc[last_val_index + 1:]
    ...
    df_test.to_pickle(output_file[2])
    ...
    with open(log_file_path, 'a') as log_file:
        ...
        log_file.write(f"  - Linhas benignas ('R'): {df_test['Label'].value_counts().get('R', 0)}\n")
        log_file.write(f"  - Linhas maliciosas ('T'): {df_test['Label'].value_counts().get('T', 0)}\n\n")
    ...
    del df, df_test , df_train, X_val_attack
