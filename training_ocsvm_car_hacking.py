...
ABSOLUTE_MODELS_PATH = ABSOLUTE_PATH + 'models/car_hacking/'
ABSOLUTE_PLOTS_PATH = ABSOLUTE_PATH + 'plots/car_hacking/'
...
if COMPLETE_TRAINING:
    ...
    log_file_path = os.path.join(ABSOLUTE_MODELS_PATH, 'complete/ocsvm_summary_car_hacking.txt')
    ...
    # --- Preparando o arquivo de log para o resumo ---
    log_file_path = os.path.join(ABSOLUTE_MODELS_PATH, 'fast/ocsvm_summary_car_hacking.txt')
    with open(log_file_path, 'w') as log_file:
        log_file.write("Resumo do treinamento dos modelos OCSVM\n")
        log_file.write("=" * 40 + "\n")
...
# --- Opcional: Reamostragem para testes rápidos dentro do script de treinamento ---
            ...
            for n in NU:
                for g in GAMMA:
                    for fold in range(FOLD_SPLIT):
                        ...
                        print(f"{dataset} - Modelo: OCSVM_{feature_set_name} | Kernel: {k} | Nu: {n} | Gamma: {g} | F1-Score: {score}")
    ...
    print("-" * 30)
    print(f"\nMelhor modelo geral para {dataset}: {overall_best_feature_set} com kernel: {overall_best_model.kernel}, nu: {overall_best_model.nu}, gamma: {overall_best_model.gamma} | Melhor F1-Score na Validação: {overall_best_f1:.4f}")
    print("-" * 30)
    with open(log_file_path, 'a') as log_file:  
        log_file.write(f"\nMelhor modelo geral para {dataset}: {overall_best_feature_set} com kernel: {overall_best_model.kernel}, nu: {overall_best_model.nu}, gamma: {overall_best_model.gamma} | Melhor F1-Score na Validação: {overall_best_f1:.4f}\n")
        log_file.write("-" * 30 + "\n")
    ...
    if COMPLETE_TRAINING:
        os.makedirs(ABSOLUTE_MODELS_PATH + 'complete/', exist_ok=True)
        output_models_file = f'{ABSOLUTE_MODELS_PATH}complete/best_ocsvm_models_car_hacking_{dataset}.pkl'
    else:
        os.makedirs(ABSOLUTE_MODELS_PATH + 'fast/', exist_ok=True)
        output_models_file = f'{ABSOLUTE_MODELS_PATH}fast/best_ocsvm_models_car_hacking_{dataset}.pkl'
    ...
    for i, (feature_set_name, model_info) in enumerate(best_models.items()):
        ...
        ax.set_title(f'Matriz de Confusão (Teste) para ocsvm_{feature_set_name} - {dataset} Attack\nF1-Score: {f1:.4f}')
    ...
    if COMPLETE_TRAINING:
        ...
        plt.savefig(f'{ABSOLUTE_PLOTS_PATH}complete/confusion_matrices_test_ocsvm_car_hacking_{dataset}.png')
        print(f"\nMatrizes de confusão para cada conjunto de features salvas em 'plots/car_hacking/complete/confusion_matrices_test_ocsvm_car_hacking_{dataset}.png'")
    else:
        os.makedirs(ABSOLUTE_PLOTS_PATH + 'fast/', exist_ok=True)
        plt.savefig(f'{ABSOLUTE_PLOTS_PATH}fast/confusion_matrices_test_ocsvm_car_hacking_{dataset}.png')
        print(f"\nMatrizes de confusão para cada conjunto de features salvas em 'plots/car_hacking/fast/confusion_matrices_test_ocsvm_car_hacking_{dataset}.png'")
